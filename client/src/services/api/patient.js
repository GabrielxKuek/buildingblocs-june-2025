...
export const getPatientItems = async () => {
    try {
        ...
        // Add images (these can have action videos)
        if (imagesData.images && Array.isArray(imagesData.images)) {
            imagesData.images.forEach((image, index) => {
                items.push({
                    id: `image_${image.media_id || index + 1}`,
                    media_id: image.media_id, // Include media_id for fetching actions
                    name: image.prompt || image.media_name || `Image ${index + 1}`,
                    imageUrl: image.media_url,
                    type: "image",
                    createdAt: image.created_at || new Date().toISOString(),
        ...
        // Add standalone videos (videos without parents)
        if (videosData.videos && Array.isArray(videosData.videos)) {
            videosData.videos
                .filter(video => !video.parent_media_id) // Only standalone videos
                .forEach((video, index) => {
                    items.push({
                        id: `video_${video.media_id || index + 1}`,
...
// NEW: Get videos associated with a specific image
export const fetchVideosByParentId = async (parentId) => {
    try {
        console.log('Fetching videos for parent ID:', parentId);
        
        const response = await fetchWithTimeout(
            getApiUrl(`/media/fetchVideosByParent/${parentId}`),
            createRequestOptions('GET'),
            getTimeoutForOperation('default')
        );

        const data = await handleApiResponse(response);
        console.log('Fetched action videos:', data);
...
export const createPatientItem = async (itemData, onProgress = null) => {
    try {
        ...
        return {
            id: `image_${Date.now()}`,
            media_id: data.mediaId, // Include media_id for future actions
            name: itemData.name,
            imageUrl: data.imageUrl,
            type: "image",
            createdAt: new Date().toISOString(),
...
export const createPatientVideo = async (itemData, onProgress = null) => {
    try {
        ...
        const response = await fetchWithTimeout(
            ...
            createRequestOptions('POST', {
                prompt: itemData.name,
                imageId: itemData.imageId || null // Use imageId for parent relationship
            }),
            getTimeoutForOperation(API_CONFIG.GENERATION_TYPES.VIDEO)
        );
